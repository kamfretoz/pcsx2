...
static void resizeBackgroundImage(QImage* image, int expected_width, int expected_height, bool fill_with_top_left)
{
	...
	if (image_width < dpr_expected_width)
		xoffset = static_cast<int>(static_cast<qreal>((dpr_expected_width - image_width) / 2) / dpr);
	if (image_height < dpr_expected_height)
		yoffset = static_cast<int>(static_cast<qreal>((dpr_expected_height - image_height) / 2) / dpr);
...
void GameListWidget::setCustomBackground(int width, int height, bool reload)
{
	...
	resizeBackgroundImage(&m_background_image, width, height, true);
